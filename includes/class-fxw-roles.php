...
class FXW_Roles
{
    ...
    public function render_agent_phone_field($user)
    {
        ...
        $phone = get_user_meta($user->ID, 'fxw_agent_phone', true);
        ?>
        <h2><?php esc_html_e('FoodXpress Agent', 'foodxpress'); ?></h2>
        <table class="form-table" role="presentation">
            <tr>
                <th>
                    <label for="fxw_agent_phone"><?php esc_html_e('Agent mobile number', 'foodxpress'); ?></label>
                </th>
                <td>
                    <input type="tel" id="fxw_agent_phone" name="fxw_agent_phone" class="regular-text"
                        value="<?php echo esc_attr($phone); ?>"
                        autocomplete="tel" inputmode="tel" />
                    <p class="description">
                        <?php esc_html_e('Shown to the customer on order tracking while this agent has the order. Use the full dialable number, e.g. +91 98765 43210.', 'foodxpress'); ?>
                    </p>
                </td>
            </tr>
        </table>
    ...
    public function save_agent_phone_field($user_id)
    {
        ...
        if (!isset($_POST['fxw_agent_phone'])) {
            return;
        }

        $phone = sanitize_text_field(wp_unslash($_POST['fxw_agent_phone']));
        if ('' === $phone) {
            delete_user_meta($user_id, 'fxw_agent_phone');
            return;
        }

        update_user_meta($user_id, 'fxw_agent_phone', $phone);
    }
}
