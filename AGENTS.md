...
- **Current version:** **v1.2.15** (2026-08-17 — v1.2.2–1.2.14: Zomato/Swiggy-style checkout, coordinates-only fee engine + saved-address defaults, WC-compat hardening, dead-code cleanup, privacy integration, config health warning, region-neutral defaults; **v1.2.15: full-flow audit fixes** — REST session bootstrap (checkout could not complete), Pricing Rules/Opening Hours settings render + save, receipt printing on AJAX + frontend paths, saved-address hook, version-aware blocks compat declaration, agent assignment email notification, reorder variations; repo public, GPL-3.0-or-later. See CHANGELOG.md)
