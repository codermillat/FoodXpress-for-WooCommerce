 ...
 *  - user meta `_fxw_delivery_profile` (pin lat/lng, exact address,
...
class FXW_Privacy
{
    ...
    private $order_meta_keys = array(
        '_fxw_address_details'    => 'Exact Delivery Address',
        '_fxw_landmark'           => 'Delivery Landmark',
        '_fxw_delivery_instructions' => 'Delivery Instructions',
        '_fxw_delivery_address'   => 'Full Delivery Address',
        '_fxw_delivery_lat'       => 'Delivery Location Latitude',
        '_fxw_delivery_lng'       => 'Delivery Location Longitude',
        '_fxw_house_flat_no'      => 'House / Flat No. (legacy)',
        '_fxw_floor_no'           => 'Floor No. (legacy)',
        '_fxw_society_building'   => 'Society / Building Name (legacy)',
        '_fxw_block_tower_area'   => 'Block / Tower / Area (legacy)',
    );

    /**
     * Keys of the `_fxw_delivery_profile` user meta, with export labels.
     *
     * @var array<string, string>
     */
    ...
    {
        $exporters['foodxpress-delivery-profile'] = array(
            'exporter_friendly_name' => __('FoodXpress Delivery Profile', 'foodxpress'),
    ...
    {
        $erasers['foodxpress-delivery-profile'] = array(
            'eraser_friendly_name' => __('FoodXpress Delivery Profile', 'foodxpress'),
    ...
    public function export_profile($email_address, $page = 1)
    {
        ...
        if ($user) {
            $profile = get_user_meta($user->ID, '_fxw_delivery_profile', true);
            if (is_array($profile)) {
                $rows = array();
                ...
                if (!empty($rows)) {
                    $data[] = array(
                        'group_id' => 'foodxpress-delivery-profile',
                        'group_label' => __('FoodXpress Delivery Profile', 'foodxpress'),
    ...
    /**
     * Append FoodXpress order meta to WooCommerce's order data export.
     *
    ...
    /**
     * Remove FoodXpress personal data from an order being anonymized.
     *
