# FoodXpress-for-WooCommerce — Feature Backport Roadmap

**Source plan:** [ANALYSIS.md](./ANALYSIS.md) (3-repo comparison)
**Goal:** Port the 17 unique features from archived `RestroReach` + `restaurant-delivery-manager` into the active `FoodXpress-for-WooCommerce` repo, in the right order, following FX's existing architecture conventions.
...
├── class-fxw-shipping-method.php               (230)
├── class-fxw-delivery-boy-view.php             (157)
...
2. **Custom DB tables use `fxw_` prefix, not `rr_` or `rdm_`.** Migration must rename on install.
3. **Settings live in the existing `FXW_Settings` page** (WooCommerce → Settings → FoodXpress). Do not create separate settings pages.
...
6. **WP-Cron, not external schedulers** — schedules land in the existing `fxw_init_plugin()` activation flow.
...
All use prefix `{$wpdb->prefix}fxw_`. Migration script registers them on activation, drops them on uninstall.

| Table | Purpose | Source (RR/RDM) |
|---|---|---|
| `fxw_delivery_agents` | Agent profiles: user_id, phone, vehicle_type, status, rating, **outlet_id** (nullable = any outlet), created_at | `rr_delivery_agents` + `outlet_id` |
| `fxw_order_assignments` | order_id → agent_id workflow with timestamps per state; `outlet_id` column | `rr_order_assignments` + `outlet_id` |
| `fxw_location_tracking` | GPS trail: agent_id, lat, lng, accuracy, battery_level, recorded_at | `rr_location_tracking` |
| `fxw_delivery_notes` | Order-level internal notes (not customer-visible) | `rr_delivery_notes` |
| `fxw_outlets` | **One or more physical locations of the same brand.** Fields: name, address, lat, lng, phone, hours_json, polygon_json, base_fee_override, per_km_fee_override, is_active, sort_order, is_default. Always has at least one row. | renamed from `rr_delivery_areas`; redesigned for owner-controlled outlet model |
| `fxw_payment_transactions` | COD audit trail: order_id, agent_id, expected, collected, change, variance | `rr_payment_transactions` |
| `fxw_cash_reconciliation` | Daily agent cash close: agent_id, date, opening, closing, variance, status | `rr_cash_reconciliation` |
| `fxw_notification_queue` | Multi-channel outbox (email + browser only at launch): channel, recipient, payload, status, retries | `rr_notification_queue` |

Indexes: all FK columns indexed, `(agent_id, recorded_at)` composite for location lookups, `(date, agent_id)` composite for reconciliation, `(is_active, sort_order)` for outlet listing, `(outlet_id, status)` on `fxw_order_assignments` for the chain-mode assignment queries, **unique partial index `is_default = 1`** on `fxw_outlets` (enforces single default at the DB level).

**The `fxw_outlets` table always has at least one row** — even on a fresh install with single-outlet mode, the migration creates the "default outlet" from the current FX restaurant-address setting. The "Number of outlets" setting in the FoodXpress dashboard only changes the **UI** (single panel vs full manager), never the schema.

### 2.2 New top-level files (target structure)

```
foodxpress-for-woocommerce.php                  (existing, no change)
uninstall.php                                   (extend to drop new tables)

includes/
├── database/                                   ← NEW
│   ├── class-fxw-schema.php                    (table definitions, install/upgrade routines)
│   └── class-fxw-migration.php                 (rr_/rdm_ → fxw_ table rename + data copy)
│
├── models/                                     ← NEW
│   ├── class-fxw-agent.php                     (CRUD on fxw_delivery_agents)
│   ├── class-fxw-assignment.php                (CRUD on fxw_order_assignments, includes outlet_id)
│   ├── class-fxw-location.php                  (CRUD on fxw_location_tracking + battery logic)
│   ├── class-fxw-outlet.php                    (CRUD on fxw_outlets + polygon helpers — chain mode)
│   ├── class-fxw-payment.php                   (CRUD on fxw_payment_transactions)
│   ├── class-fxw-reconciliation.php            (CRUD on fxw_cash_reconciliation)
│   └── class-fxw-notification-queue.php        (CRUD on fxw_notification_queue + retry)
│
├── services/                                   ← extend
│   ├── class-fxw-mapping-service.php           (existing)
│   ├── class-fxw-rate-limiter.php              (existing)
│   ├── class-fxw-cod-calculator.php            ← NEW (calculate_change, variance)
│   ├── class-fxw-distance-cache.php            ← NEW (24h geocoding, 7d coord cache)
│   └── class-fxw-report-scheduler.php          ← NEW (WP-Cron registration)
│
├── api/                                        ← extend
│   ├── class-fxw-rest-checkout-controller.php  (existing)
│   ├── class-fxw-rest-agent-controller.php     ← NEW (agent profile, location, status)
│   ├── class-fxw-rest-order-controller.php     ← NEW (assign, status updates, notes)
│   ├── class-fxw-rest-payment-controller.php   ← NEW (collect COD, reconcile)
│   └── class-fxw-rest-analytics-controller.php ← NEW (chart data, exports)
│
├── notifications/                              ← NEW (split from fxw-notifications.php)
│   ├── class-fxw-notification-channels.php     (registry: email, browser, sms, whatsapp)
│   ├── class-fxw-notification-email.php        (moved from includes/emails/, expanded)
│   ├── class-fxw-notification-browser.php      (Web Push via service worker)
│   ├── class-fxw-notification-sms.php          (gateway abstraction, no provider lock-in)
│   └── class-fxw-notification-queue-processor.php (cron worker)
│
├── analytics/                                  ← NEW
│   ├── class-fxw-analytics-revenue.php         (revenue trends, period comparison)
│   ├── class-fxw-analytics-agents.php          (agent KPIs, ratings)
│   ├── class-fxw-analytics-delivery.php        (delivery time, peak hours)
│   ├── class-fxw-analytics-customers.php       (satisfaction, repeat rate)
│   └── class-fxw-analytics-export.php          (CSV/JSON export)
│
├── pwa/                                        ← NEW
│   ├── class-fxw-pwa-manifest.php              (programmatic manifest generation)
│   └── class-fxw-pwa-service-worker.php        (SW registration + cache strategies)
│
├── admin/                                      ← NEW
│   ├── class-fxw-admin-cash-reconciliation.php (the reconciliation page from RR)
│   ├── class-fxw-admin-analytics-dashboard.php (charts view)
│   ├── class-fxw-admin-agent-roster.php        (agent list + rating + assign, filtered by outlet)
│   ├── class-fxw-admin-outlets.php             (multi-outlet chain editor — Google Maps + Leaflet fallback)
│   └── class-fxw-admin-tools.php               (DB test, sample data, repair)
│
├── class-fxw-core.php                          (extended: register new services, crons, REST routes)
├── class-fxw-checkout.php                      ← REFACTOR (split from 1,046 LOC into 3 files)
├── class-fxw-roles.php                         (extended with agent capabilities from RR)
└── class-fxw-notifications.php                 (becomes a thin facade → notifications/*)
...
```php
define('FXW_DB_VERSION', '2.0.0');             // bumped on schema change
define('FXW_GPS_INTERVAL_SEC', 45);             // matches RR
define('FXW_GPS_BATTERY_LOW', 20);              // throttle when battery < 20%
define('FXW_GEOFENCE_CACHE_GEOCODE_TTL', 86400);// 24h
define('FXW_GEOFENCE_CACHE_COORDS_TTL', 604800);// 7d
define('FXW_COD_DAILY_CRON_HOOK', 'fxw_daily_cash_reconciliation');
define('FXW_ANALYTICS_DAILY_HOOK', 'fxw_daily_analytics_report');
define('FXW_ANALYTICS_WEEKLY_HOOK', 'fxw_weekly_analytics_report');
define('FXW_ANALYTICS_MONTHLY_HOOK', 'fxw_monthly_analytics_report');
define('FXW_CLEANUP_LOCATION_HOOK', 'fxw_cleanup_old_locations');

// Owner-controlled outlet model (Phase 7)
define('FXW_OPERATION_MODE_SINGLE', 'single');  // default
define('FXW_OPERATION_MODE_MULTI', 'multi');
// Stored in wp_options as 'fxw_operation_mode' — read via get_option('fxw_operation_mode', FXW_OPERATION_MODE_SINGLE)
```
...
**Why first:** every later feature needs a place to store data. RR's tables become FXW tables, with a one-time migration that renames `{$wpdb->prefix}rr_*` → `{$wpdb->prefix}fxw_*` and copies data.

**Tasks:**

- [ ] Create `includes/database/class-fxw-schema.php` with `install()`, `upgrade()`, `table_names()` (centralized so no class hard-codes table names)
- [ ] Create `includes/database/class-fxw-migration.php` with:
...
- [ ] Wire schema install/upgrade into `fxw_activate()` (main bootstrap)
- [ ] Add `fxw_db_version` option to track schema version
- [ ] Extend `uninstall.php` to drop all `fxw_*` tables (only if user opts in via settings checkbox)
- [ ] Add models in `includes/models/`:
  - `class-fxw-agent.php` — `create()`, `find()`, `update()`, `delete()`, `get_by_user_id()`, `get_active()`
  - `class-fxw-assignment.php` — `assign()`, `unassign()`, `get_by_order()`, `get_by_agent()`, `mark_picked_up()`, `mark_delivered()`
  - `class-fxw-location.php` — `record()`, `get_latest_for_agent()`, `get_trail()`, `cleanup_older_than()`
- [ ] Add tests in `tests/test-database.php`

**Schema details** (from RR, with improvements):

```sql
CREATE TABLE {fxw_delivery_agents} (
...
- Fresh install: 8 tables created in `{$wpdb->prefix}fxw_*`
...
- [ ] Create `assets/pwa/manifest.json` (programmatic in `class-fxw-pwa-manifest.php`, but ship a static fallback)
...
- [ ] Add `class-fxw-pwa-service-worker.php` to enqueue SW registration script + handle `?fxw_pwa=1` query
...
- [ ] Add `wp_ajax_fxw_upload_delivery_photo` handler (uses `wp_handle_upload`)
...
**Why third:** needs the PWA shell from Phase 2 to be useful, and the `fxw_location_tracking` table from Phase 1.

**Tasks:**

- [ ] Create `includes/models/class-fxw-location.php` (already stubbed in Phase 1 — flesh it out)
- [ ] Create `includes/services/class-fxw-distance-cache.php` (24h geocoding / 7d coords cache using WordPress transients)
- [ ] Create `includes/api/class-fxw-rest-agent-controller.php` with:
...
- [ ] Add `class-fxw-customer-tracking.php` to merge into existing `class-fxw-shortcodes.php` (which already has `[fxw_track_order]`)
- [ ] Add `assets/js/live-tracking-map.js` (extends the existing tracking shortcode to show agent's live position)
- [ ] Add `wp_cron` schedule: `fxw_cleanup_old_locations` (daily) — deletes trail points older than 30 days
...
- [ ] Create `includes/models/class-fxw-payment.php`
- [ ] Create `includes/models/class-fxw-reconciliation.php`
- [ ] Create `includes/services/class-fxw-cod-calculator.php`:
...
- [ ] Create `includes/api/class-fxw-rest-payment-controller.php`:
...
- [ ] Create `includes/admin/class-fxw-admin-cash-reconciliation.php` (port the UI from RR's `class-payments.php::render_cash_reconciliation_page()`)
...
- [ ] Register `fxw_daily_cash_reconciliation` WP-Cron at activation (daily 23:55)
- [ ] Add `wp_ajax_fxw_*` handlers for the admin UI
...
- [ ] Create `includes/models/class-fxw-notification-queue.php` (CRUD + retry logic)
- [ ] Create `includes/notifications/class-fxw-notification-channels.php` (channel registry: `['email', 'browser']` — **SMS intentionally dropped at launch**, see decisions below)
- [ ] Refactor existing 3 email classes (`class-fxw-email-*.php`) into `includes/notifications/class-fxw-notification-email.php` with a `WC_Email` base
- [ ] Create `includes/notifications/class-fxw-notification-browser.php`:
  - Web Push via `class-fxw-pwa-service-worker.php` from Phase 2
  - Uses VAPID keys (settings page field)
  - Subscription storage: `wp_user_meta` (`fxw_push_subscription`)
- [ ] ~~`class-fxw-notification-sms.php`~~ — **REMOVED**. SMS was in scope earlier; user decided no SMS at launch. The channel interface is still in place so SMS can be added later as a 3rd channel without changing the rest of the system (see §6 decisions).
- [ ] Create `includes/notifications/class-fxw-notification-queue-processor.php` (cron worker that drains the queue every 5 minutes)
...
```php
interface FXW_Notification_Channel {
...
class FXW_Notification_Email   implements FXW_Notification_Channel { ... }
class FXW_Notification_Browser implements FXW_Notification_Channel { ... }
// FXW_Notification_SMS: not in v1.7.0 — interface allows adding it later without touching the rest
```
...
- [ ] Create `includes/analytics/class-fxw-analytics-revenue.php` (port from RR's `get_revenue_analytics`)
- [ ] Create `includes/analytics/class-fxw-analytics-agents.php` (port `get_agent_performance`, `get_agent_rating`)
- [ ] Create `includes/analytics/class-fxw-analytics-delivery.php` (port `get_delivery_time_analytics`, `get_peak_hours_analytics`)
- [ ] Create `includes/analytics/class-fxw-analytics-customers.php` (port `get_customer_satisfaction`, repeat-customer rate)
- [ ] Create `includes/analytics/class-fxw-analytics-export.php` (CSV + JSON export)
- [ ] Create `includes/api/class-fxw-rest-analytics-controller.php` (chart data endpoints)
- [ ] Create `includes/admin/class-fxw-admin-analytics-dashboard.php` (admin page with Chart.js)
...
- [ ] Create `includes/services/class-fxw-report-scheduler.php`:
  - `fxw_daily_analytics_report` — 8 AM daily
  - `fxw_weekly_analytics_report` — Monday 8 AM
  - `fxw_monthly_analytics_report` — 1st of month 8 AM
...
**Why seventh:** smaller wins that complete the picture. Adds the schema and admin UI to support chains with multiple physical outlets of the same brand. **Single-outlet is the default; the owner can switch to multi-outlet any time from the FoodXpress settings page — no reinstall, no data loss.**
...
- The schema always uses the outlet model — there's always at least one `fxw_outlets` record
- A single-outlet setup is just N=1 — no special-casing in the shipping method, no migration when switching
- The FoodXpress settings page exposes a toggle: **"Number of outlets"** = `1` | `Many`
...
- [ ] Create `includes/models/class-fxw-outlet.php` (CRUD + polygon helpers)
...
- [ ] Add `outlet_id` column to `fxw_order_assignments` (so each assignment knows which outlet is fulfilling it)
- [ ] Add `outlet_id` column to `fxw_delivery_agents` (so each agent can be tied to a specific outlet; NULL = "any outlet")
- [ ] On Phase-1 migration: auto-create one `fxw_outlets` record from the current FX restaurant-address setting, mark it `is_default = 1`
- [ ] Create `includes/services/class-fxw-outlet-selector.php`:
  - `find_outlet_for_address($address): ?FXW_Outlet` (returns nearest open outlet whose radius/polygon contains the address)
  - `get_open_outlets(): array` (filters by `is_active` + `hours_json`)
  - `get_default_outlet(): ?FXW_Outlet` (returns the `is_default = 1` record; used in single-outlet mode)
- [ ] Create `includes/admin/class-fxw-admin-outlets.php`:
  ...
  - Conditional rendering based on settings: `fxw_operation_mode` ('single' | 'multi')
- [ ] Create `templates/admin/outlets.php` (list — only shown when `fxw_operation_mode = 'multi'`) and `templates/admin/outlet-edit.php` (editor)
- [ ] Create `assets/js/outlets-editor.js`:
  - **Primary path:** Google Maps JS API + `google.maps.Polygon` + `google.maps.drawing.DrawingManager`
  - **Fallback path:** if `fxw_google_maps_api_key` setting is empty, load Leaflet + Leaflet.draw from CDN as fallback
  - Both paths share the same `polygon_json` schema output
- [ ] Create `assets/css/outlets-editor.css`
- [ ] Add a settings field: **"Number of outlets"** (radio: `1` | `Many`) — stored as `fxw_operation_mode` ('single' | 'multi'). Default: 'single'. Changing it is instant, no data migration.
- [ ] Refactor `FXW_Shipping_Method` to always use `FXW_Outlet_Selector` — when only 1 outlet exists, the selector returns it trivially; no separate code path
- [ ] Add per-outlet agent assignment UI (existing `class-fxw-admin-agent-roster.php` gets an outlet filter; agents with `outlet_id = NULL` show in "Any outlet")
- [ ] Create `includes/admin/class-fxw-admin-agent-roster.php` (agent list, ratings, assign-to-order UI, with outlet filter)
- [ ] Create `templates/admin/agent-roster.php`
- [ ] Create `assets/js/agent-roster.js`
- [ ] Port RR's `class-rdm-admin-tools.php` (good idea, not the bloat) → `class-fxw-admin-tools.php`:
...
- [ ] Final security audit: `class-fxw-database-security.php` review (manual code review, not just scan)
...
| 7 — Multi-outlet (chain) mode + admin | 6 | 3 | 2 | 3 | 0 (uses `fxw_outlets` from Phase 1) | 1 wk | 1.9.0 |
...
```
1. Activate v1.3.0
2. fxw_install() runs → creates 8 fxw_* tables
3. fxw_migrate_from_meta() scans WC order meta for existing orders
4. For each WC order with `_fxw_status` meta: create fxw_order_assignments row
5. Done — no data loss, no broken orders

(When v1.9.0 lands and runs fxw_install's outlet step:)
6. fxw_create_default_outlet() reads the current FX restaurant-address setting
7. Creates one fxw_outlets record with is_default=1, is_active=1
...
```
1. Activate v1.3.0
2. fxw_install() detects existing `{$wpdb->prefix}rr_*` tables
3. fxw_migrate_from_rr() renames each `rr_*` → `fxw_*` (preserves data)
4. Any saved settings (wp_options like `rdm_*`) are copied to `fxw_*` keys
...
| Q | Phase | Status |
|---|---|---|
| ~~Single restaurant vs multi-location default?~~ | 7 | **✅ RESOLVED 2026-08-16: owner-configurable from the dashboard.** Single-outlet is the default; owner can switch to multi-outlet any time from the FoodXpress settings page. Schema always uses the outlet model (single-outlet is just N=1). |
...
The local clones at `~/Desktop/FoodXpress for WooCommerce/` and `~/Local Sites/foodxpress-for-woocommerce/` can be cleaned up at the same time.
