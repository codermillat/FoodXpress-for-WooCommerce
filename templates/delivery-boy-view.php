					...
					<div class="fxw-order-card">
						...
						<div class="fxw-card-body">
							...
							<div class="fxw-card-section">
								...
								<p><strong><?php _e('Method:', 'foodxpress'); ?></strong>
									<?php echo esc_html($order->get_payment_method_title()); ?></p>
								<?php if ('cod' === $order->get_payment_method()): ?>
									<p class="fxw-collect-amount"><strong><?php _e('Collect:', 'foodxpress'); ?></strong>
										<?php echo wp_kses_post($order->get_formatted_order_total()); ?></p>
								<?php endif; ?>
							</div>
						</div>
						<div class="fxw-card-footer">
							<a href="https://www.google.com/maps/search/?api=1&query=<?php echo urlencode(strip_tags($shipping_address)); ?>"
								target="_blank"
								class="fxw-button button-secondary"><?php _e('Open in Map', 'foodxpress'); ?></a>
							<?php if ('fxw-assigned' === $status): ?>
								<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
									...
									<button type="submit"
										class="fxw-button button-primary"><?php _e('Mark Picked Up', 'foodxpress'); ?></button>
								</form>
							<?php endif; ?>
							<?php if ('fxw-picked-up' === $status): ?>
								<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
									...
									<input type="hidden" name="order_id" value="<?php echo esc_attr($order->get_id()); ?>" />
									<button type="submit"
										class="fxw-button button-primary"><?php _e('Mark Delivered', 'foodxpress'); ?></button>
								</form>
							<?php endif; ?>
						</div>
					</div>
