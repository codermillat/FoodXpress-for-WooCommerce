<?php
/**
 * FXW_Settings_Extra — Settings extension points.
 *
 * Self-registers three self-contained settings affordances that previously
 * bloated the main FXW_Settings class past its 500-LOC cap (1.2.16):
 ...
 * settings code already uses (woocommerce_update_options_foodxpress +
 * fxw_sanitize_settings_extra), so FXW_Settings remains the single owner
 * of the tab.
 *
 * @since      1.2.16
 * @package    FoodXpress
...
class FXW_Settings_Extra
{
	...
	public function __construct()
	{
		...
		add_action('woocommerce_update_options_foodxpress', array($this, 'verify_capability'), 1);
		// Run after the core sanitize + the other extensions so the
		// transient + preserve list land at the end of the sanitize.
		add_filter('fxw_sanitize_settings_extra', array($this, 'sanitize_extra'), 100, 2);
		// Preserve fxw_is_open + fxw_remove_on_uninstall when the form
		// does not post them.
		add_filter('fxw_sanitize_settings_extra', array($this, 'preserve_keys'), 999, 2);
	}
	...
	public function verify_capability()
	{
		if (!current_user_can('manage_woocommerce')) {
			...
			// requires.
			remove_action('woocommerce_update_options_foodxpress', 'FXW_Settings::save_wc_settings_tab', 10);
		}
	}
	...
	public function sanitize_extra($sanitized, $input)
	{
		...
		if (isset($input['fxw_restaurant_latlng'])) {
			...
			if (preg_match('/^-?\d{1,3}(\.\d+)?\s*,\s*-?\d{1,3}(\.\d+)?$/', $raw) && abs($lat) <= 90 && abs($lng) <= 180) {
				$sanitized['fxw_restaurant_latlng'] = $raw;
			} else {
				set_transient('fxw_admin_notice', __('Restaurant Coordinates were out of range or malformed; previous value kept.', 'foodxpress'), 30);
			}
		}

		// Uninstall opt-in: 'yes' when checked, 'no' when absent.
		$sanitized['fxw_remove_on_uninstall'] = isset($input['fxw_remove_on_uninstall']) ? 'yes' : 'no';
	...
	public function preserve_keys($sanitized, $input)
	{
		...
		foreach (array('fxw_is_open', 'fxw_remove_on_uninstall') as $key) {
