...
- **Current version:** **v1.2.16** (2026-08-17 — v1.2.2–1.2.15: Zomato/Swiggy-style checkout, coordinates-only fee engine + saved-address defaults, WC-compat hardening, dead-code cleanup, privacy integration, config health warning, region-neutral defaults, full-flow audit fixes (REST session bootstrap, settings sections render, receipt printing, saved-address hook, blocks compat, agent email); **v1.2.16: 16-minor cleanup pass** — REST schedule parity, no-API-key message, unassign status revert, agent transition enforcement, wp-login redirect, track-order rate limit, reorder feedback, settings (fractional radius, latlng range, uninstall opt-in, explicit save cap), cart-block notices, reporting site-timezone, dashboard `fxw-in-kitchen` whitelist, dead-code removal; repo public, GPL-3.0-or-later. See CHANGELOG.md)
