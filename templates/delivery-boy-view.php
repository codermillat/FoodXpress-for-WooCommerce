...
<div class="fxw-delivery-dashboard">
	<div class="fxw-container">
		<h1><?php esc_html_e('My Assigned Deliveries', 'foodxpress'); ?></h1>
		<?php
					...
					<div class="fxw-order-card">
						...
						<div class="fxw-card-body">
							<div class="fxw-card-section">
								<h4><?php esc_html_e('Customer Details', 'foodxpress'); ?></h4>
								<p><strong><?php esc_html_e('Name:', 'foodxpress'); ?></strong>
									<?php echo esc_html($order->get_formatted_billing_full_name()); ?></p>
								<p><strong><?php esc_html_e('Address:', 'foodxpress'); ?></strong>
									<?php echo wp_kses_post($shipping_address); ?></p>
								<?php if ($unit): ?>
									<p><strong><?php esc_html_e('Unit:', 'foodxpress'); ?></strong> <?php echo esc_html($unit); ?>
									</p>
								<?php endif; ?>
								<p><strong><?php esc_html_e('Phone:', 'foodxpress'); ?></strong> <a
										href="tel:<?php echo esc_attr($order->get_billing_phone()); ?>"><?php echo esc_html($order->get_billing_phone()); ?></a>
								</p>
							</div>
							<div class="fxw-card-section">
								<h4><?php esc_html_e('Payment Information', 'foodxpress'); ?></h4>
								<p><strong><?php esc_html_e('Method:', 'foodxpress'); ?></strong>
									<?php echo esc_html($order->get_payment_method_title()); ?></p>
								<?php if ('cod' === $order->get_payment_method()): ?>
									<p class="fxw-collect-amount"><strong><?php esc_html_e('Collect:', 'foodxpress'); ?></strong>
						...
						<div class="fxw-card-footer">
							<a href="<?php echo esc_url('https://www.google.com/maps/search/?api=1&query=' . urlencode(wp_strip_all_tags($shipping_address))); ?>"
								...
								class="fxw-button button-secondary"><?php esc_html_e('Open in Map', 'foodxpress'); ?></a>
							<?php if ('fxw-assigned' === $status): ?>
								<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
									...
									<button type="submit"
										class="fxw-button button-primary"><?php esc_html_e('Mark Picked Up', 'foodxpress'); ?></button>
								</form>
							<?php endif; ?>
							<?php if ('fxw-picked-up' === $status): ?>
								<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
									...
									<input type="hidden" name="order_id" value="<?php echo esc_attr($order->get_id()); ?>" />
									<button type="submit"
										class="fxw-button button-primary"><?php esc_html_e('Mark Delivered', 'foodxpress'); ?></button>
								</form>
							<?php endif; ?>
						</div>
					</div>
		...
		<?php else: ?>
			<div class="fxw-no-orders">
				<p><?php esc_html_e('You have no assigned deliveries at the moment.', 'foodxpress'); ?></p>
			</div>
		<?php endif; ?>
	</div>
</div>
