...
public function __construct()
    {
        ...
        add_action('wp_ajax_fxw_agent_dashboard_state', array($this, 'ajax_dashboard_state'));
        add_action('admin_post_fxw_settle_agent_cash', array($this, 'handle_cash_settlement'));
        add_action('admin_post_fxw_review_settlement', array($this, 'handle_settlement_review'));
     ...
     * `?fxw_agent_manifest=1` returns the web-app manifest and
     * `?fxw_agent_sw=1` streams the bundled service worker with a
    ...
    public function serve_pwa_endpoints()
    {
        ...
        if (!isset($_GET['fxw_agent_manifest']) && !isset($_GET['fxw_agent_sw'])) {
            return;
        }

        if (isset($_GET['fxw_agent_manifest'])) {
            $manifest = array(
                'name' => __('FoodXpress Agent', 'foodxpress'),
                'short_name' => __('FX Agent', 'foodxpress'),
                'description' => __('Delivery agent dashboard for FoodXpress orders.', 'foodxpress'),
                ...
                'icons' => array(
                    array(
                        'src' => FXW_PLUGIN_URL . 'assets/img/agent-icon.svg',
                        'sizes' => 'any',
                        'type' => 'image/svg+xml',
                        'purpose' => 'any',
                    ),
                    array(
                        'src' => FXW_PLUGIN_URL . 'assets/img/agent-icon-maskable.svg',
                        'sizes' => 'any',
                        'type' => 'image/svg+xml',
                        'purpose' => 'maskable',
                    ),
        ...
        $sw_path = FXW_PLUGIN_DIR . 'assets/js/fxw-agent-sw.js';
    ...
    {
        check_ajax_referer('fxw_agent_state', 'nonce');
    ...
    public static function build_dashboard_state($agent_id)
    {
        ...
        $new_orders = wc_get_orders(array_merge($args, array('status' => 'fxw-assigned')));
        $active_orders = wc_get_orders(array_merge($args, array('status' => 'fxw-picked-up')));
...
public function mark_picked_up()
	 ...
	 * amount is computed server-side by FXW_Agent_Cash. A manager or
	...
	public function handle_cash_settlement()
	{
		...
		if (!$agent_id || !wp_verify_nonce($nonce, 'fxw_settle_cash_' . $agent_id)) {
			wp_die(__('Security verification failed. Please try again.', 'foodxpress'));
		}

		if (!is_user_logged_in() || get_current_user_id() !== $agent_id || !current_user_can('fxw_delivery_access')) {
			wp_die(__('Only the delivery agent can request a hand-over of their own collected cash.', 'foodxpress'));
		}

		$amount = FXW_Agent_Cash::create_request($agent_id, get_current_user_id());
		...
		$note = sprintf(
			...
			__('Cash hand-over of %1$s requested by %2$s — awaiting manager approval.', 'foodxpress'),
	...
	public function handle_settlement_review()
	{
		...
		if (!$id || !in_array($decision, array(FXW_Agent_Cash::STATUS_ACCEPTED, FXW_Agent_Cash::STATUS_REJECTED), true)
			|| !wp_verify_nonce($nonce, 'fxw_review_cash_' . $id)) {
			wp_die(__('Security verification failed. Please try again.', 'foodxpress'));
		}

		if (!current_user_can('edit_shop_orders')) {
			wp_die(__('Only managers and admins can approve cash hand-overs.', 'foodxpress'));
		}

		$result = FXW_Agent_Cash::review_request($id, $decision, get_current_user_id());
		if (null === $result) {
			$back = wp_get_referer() ? wp_get_referer() : admin_url('admin.php?page=fxw-deliveries-dashboard');
			wp_safe_redirect($back);
			exit;
		}

		$reviewer = wp_get_current_user()->display_name;
		if (FXW_Agent_Cash::STATUS_ACCEPTED === $decision) {
			$note = sprintf(
				/* translators: 1: formatted amount, 2: reviewer name. */
				__('Cash hand-over of %1$s ACCEPTED by %2$s. Agent balance cleared.', 'foodxpress'),
			...
			$note = sprintf(
				/* translators: 1: formatted amount, 2: reviewer name. */
				__('Cash hand-over request of %1$s REJECTED by %2$s. Amount stays on the agent\'s balance.', 'foodxpress'),
				wp_strip_all_tags(wc_price($result['amount'])),
				$reviewer
			);
		}
