 ...
 * WC_Email classes registered in class-fxw-core.php trigger
...
class FXW_Notifications
{
	 ...
	 * class-fxw-core.php, customizable in WooCommerce → Settings →
	 * Emails):
	...
	public function __construct()
	{
		...
		// stores _fxw_delivery_boy_id and THEN transitions the order to
		// fxw-assigned, so the status hook covers all of them uniformly.
		add_action('woocommerce_order_status_fxw-assigned', array('FXW_Notifications', 'notify_assigned_agent'), 10, 2);
	}
	...
	public static function notify_assigned_agent($order_id, $order = false)
	{
		...
		$agent_id = (int) $order->get_meta('_fxw_delivery_boy_id', true);
		if (!$agent_id) {
		...
		if (!$agent || empty($agent->user_email) || !is_email($agent->user_email)) {
			...
			$order->add_order_note(sprintf(__('Assignment notification skipped: no valid email address for delivery agent (user #%d).', 'foodxpress'), $agent_id));
			return;
		}
		...
		$subject = sprintf(
			...
			__('[%1$s] New delivery assigned to you — Order #%2$s', 'foodxpress'),
			$site_name,
			$order_number
		);

		$message = sprintf(__('Hello %s,', 'foodxpress'), $agent->display_name) . "\r\n\r\n";
		$message .= sprintf(
			/* translators: %s: order number */
			__('You have been assigned to deliver Order #%s.', 'foodxpress'),
			$order_number
		) . "\r\n\r\n";

		$shipping_address = trim((string) $order->get_formatted_shipping_address());
		if ('' !== $shipping_address) {
			$message .= __('Delivery address:', 'foodxpress') . "\r\n";
			$message .= str_replace(array('<br/>', '<br />', '<br>'), "\r\n", $shipping_address) . "\r\n\r\n";
		}

		/* translators: %s: formatted order total */
		$message .= sprintf(__('Order total: %s', 'foodxpress'), wp_strip_all_tags($order->get_formatted_order_total())) . "\r\n";
		if ('cod' === $order->get_payment_method()) {
			$message .= __('Payment: Cash on Delivery — collect the order total from the customer.', 'foodxpress') . "\r\n";
		}

		$message .= "\r\n" . __('Your delivery dashboard:', 'foodxpress') . "\r\n" . home_url('/delivery-dashboard/') . "\r\n";

		$sent = wp_mail($agent->user_email, $subject, $message);

		if ($sent) {
			/* translators: %s: delivery agent display name */
			$order->add_order_note(sprintf(__('Assignment notification emailed to %s.', 'foodxpress'), $agent->display_name));
		} else {
			/* translators: %s: delivery agent display name */
			$order->add_order_note(sprintf(__('Could not email the assignment notification to %s.', 'foodxpress'), $agent->display_name));
		}
	}
}
