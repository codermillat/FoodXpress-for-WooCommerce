...
- **Current version:** **v1.3.9** + customer /my-account UI overhaul (unreleased, shipping in v1.4.0 — see CHANGELOG.md "Unreleased"). 2026-08-18 — v1.2.2–1.2.19: Zomato/Swiggy-style checkout, coordinates-only fee engine + saved-address defaults, WC-compat hardening, dead-code cleanup, privacy integration, config health warning, region-neutral defaults, full-flow audit fixes, 16-minor cleanup pass, REST `floatval` callback PHP 8 fatal fix, blocks-checkout `register_fields` moved to `woocommerce_blocks_loaded` for WC 11.0+, map picker on block themes via `render_block` on `woocommerce/checkout`, `is_store_open()` moved to `FXW_Store_Hours` (loaded every request), Leaflet fallback (no API key required). **v1.3.0: checkout UX cleanup** — double-charging removed (Shipping Method API sole charge), country/state/city/postcode hidden + relabelled `address_1` field ("Flat / Floor / Block / Society / Tower"), landmark label dedup, removed obsolete `fxw_enable_extra_delivery_fee` setting. **v1.3.1: blocks Store API namespace** — `woocommerce_store_api_register_update_callback` registers `foodxpress` on `woocommerce_blocks_loaded` (callback persists `customer_lat`/`customer_lng` and recomputes shipping + totals); without this registration the block checkout threw "There is no such namespace registered: foodxpress." every time the picker called `extensionCartUpdate`. **v1.3.2: live block-API detection** — picker no longer caches whether `wc.blocksCheckout.extensionCartUpdate` exists; it sniffs on every pin drag because `wc.blocksCheckout` may not be wired in at init time. **v1.3.3: critical error on block checkout** — `$order->set_address_2()` does not exist in WC 11 (`set_billing_address_2()` / `set_shipping_address_2()` are the only address-2 setters); the order persistence path took the dead call once the FXW block path reached it. Removed. **v1.3.4: shipping method provider-aware** — the previous gate `if ( empty( $options['fxw_google_maps_api_key'] ) ) return;` made `calculate_shipping()` bail on every Leaflet/OSM install (no Google key needed), so the rate was never re-added and the Order summary stayed stale on pin drag. Replaced with a `FXW_Map_Providers::supports()` capability check — **but v1.3.4 checked `supports( 'distance', ... )`, a capability no provider registers (registry enum: map/geocode/routing), so every keyless store silently produced ZERO rates; fixed in v1.3.8 to `supports( 'map', ... )` + early-return logging (found via `wp-content/uploads/wc-logs/foodxpress-*.log`: last "adding rate" entry predates v1.3.4)**. **v1.3.5: "No available delivery option" config-gap fix** — once v1.3.4 made the method actually runnable, the latent config gap (no WC shipping zone had `foodxpress_delivery` enabled) surfaced as "No available delivery option" / "No shipping options are available for this address." because WC only calls a shipping method for packages belonging to a zone that has the method enabled. New `fxw_ensure_shipping_method_registered()` walks every existing zone (named + WC's synthetic zone 0 "Rest of the world") and adds `foodxpress_delivery` idempotently; runs on activation and on the next admin page hit (6h transient; v1.3.6 also hooks frontend `init` so stores that never visit wp-admin self-heal). **v1.3.7: `reset_shipping()` before `calculate_shipping()`** in the Store API extension callback (WC_Shipping caches packages in `$this->packages`). **v1.3.8: capability gate fix + live-verified** — gate now checks `map`; every early return in `calculate_shipping()` logs to the `foodxpress` WC log source; `store_base()` falls back to the raw `woocommerce_default_country` option. Verified live in Playwright: pin drop → `FoodXpress Delivery ₹X` appears in Order summary; pin move → rate updates instantly; no banner. **v1.3.9: order UI polish** — the exact address is no longer duplicated into `address_2` (since v1.3.0 the customer types it into WC's own `address_1`, so `get_formatted_shipping_address()` = line1+line2 rendered it twice on confirmation/receipt/admin); `address_2` now carries only the landmark; one-shot guarded migration (`fxw_migrated_address2_v139` option, HPOS-safe `wc_get_orders`) normalises already-placed orders (verified in DB: orders 21–26 single-line); free-delivery rates now label themselves "(Free delivery)" instead of a blank amount (order #25 hit the ₹500 threshold). **Unreleased (v1.4.0): customer /my-account UI overhaul** — new `FXW_My_Account` class renders a FoodXpress-flavored dashboard on `/my-account/` (welcome banner with Reorder CTA, 4-card stats row, Recent orders + Default delivery address panels, colour-coded status pills matching the delivery vocabulary, empty states). New `assets/css/my-account-dashboard.css` loads only on the dashboard root (gated by `is_wc_endpoint_url()`). `my-account.css` extended with SVG `mask-image` nav icons (Dashboard grid, Orders receipt, Downloads arrow, Addresses pin, Account user, Logout arrow, Payment card) and `content: none !important` to suppress the theme-injected ">" chevron on the active item. All 141/141 tests pass. Repo public, GPL-3.0-or-later. See CHANGELOG.md)
...
- **Test runner:** `php tests/FXWTestRunner.php` → must report **138/138 pass** before any commit (v1.3.8 added two SHA-256 integrity checks for the bundled Leaflet JS/CSS)
...
│   ├── class-fxw-my-account.php       ← customer /my-account dashboard widgets (v1.4.0)
