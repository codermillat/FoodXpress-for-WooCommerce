...
- **Current version:** **v1.4.0** (released 2026-08-22 — see CHANGELOG.md). Highlights: customer /my-account dashboard overhaul (`FXW_My_Account`, `my-account-dashboard.css`), delivery-rider PWA (manifest/SW via query-string endpoints, heartbeat auto-refresh, COD strips, bottom tabs), cash settlement workflow (`FXW_Agent_Cash` ledger: agent requests → manager/admin accept/reject with audit trail), work-tracking table (`FXW_Dashboard_Agents`), settings redesign for non-tech admins (conditional visibility via `data-fxw-show-when` contract in `admin-settings.js`, masked keys, Google fields under Map Provider section), open-all-day + special-occasion override hours, admin-bar toggle acts on effective state, kitchen-note privacy split (riders see delivery instructions only; Kitchen Note column for managers), `post_status_exists()` → `get_post_status_object()` fix at 3 sites, receipt bill rebuild (`wc_price` line totals, full breakdown, comma-separated address). readme.txt with external-service disclosure + setup/API-key/security docs. Tests: **147/147** (PHP 8.5 CLI and live PHP 7.4.30 both verified). Mimosa deep scan 0 findings. Repo public, GPL-3.0-or-later. See CHANGELOG.md. Historical notes below.
