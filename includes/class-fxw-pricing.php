 ...
 *
 * Everything here is managed from the FoodXpress settings tab — nothing
 * is hardcoded:
 ...
 *    0 disables)
 *
 * Registered via the fxw_settings_register_extra_fields /
 * fxw_sanitize_settings_extra extension points; the fee decisions are
...
class FXW_Pricing
{
	...
	public function __construct()
	{
		...
		add_filter('fxw_sanitize_settings_extra', array($this, 'sanitize_pricing'), 10, 2);
		add_action('woocommerce_before_cart', array($this, 'render_minimum_order_notice'));
	}

	/**
	 * Register the Pricing Rules section on the FoodXpress settings tab.
	 *
	 * @since 1.2.13
	 */
	...
	{
		echo '<p>' . esc_html__('All delivery pricing is configured here — tiers, free delivery, and the minimum order. Amounts are in your store currency.', 'foodxpress') . '</p>';
	}
	...
	public function render_tiers_field()
	{
		...
		for ($i = 0; $i < self::TIER_ROWS; $i++) {
			...
			printf(
				'<tr><td><input type="number" step="0.1" min="0" name="fxw_settings[fxw_fee_tiers][%1$d][to]" value="%2$s" style="width:90px" /></td><td><input type="number" step="0.01" min="0" name="fxw_settings[fxw_fee_tiers][%1$d][fee]" value="%3$s" style="width:100px" /></td></tr>',
				$i,
				esc_attr($to),
				esc_attr($fee)
			);
	...
	public function render_amount_field($args)
	{
		...
		printf(
			'<input type="number" step="0.01" min="0" name="fxw_settings[%1$s]" value="%2$s" style="width:120px" /><p class="description">%3$s</p>',
			esc_attr($id),
			esc_attr($value),
			esc_html($args['hint'])
		);
	...
	public function sanitize_pricing($sanitized, $input)
	{
		...
		if ($sanitized['fxw_free_delivery_threshold'] < 0) {
			$sanitized['fxw_free_delivery_threshold'] = 0;
		}
		if ($sanitized['fxw_minimum_order'] < 0) {
			$sanitized['fxw_minimum_order'] = 0;
		}

		$tiers = array();
		if (isset($input['fxw_fee_tiers']) && is_array($input['fxw_fee_tiers'])) {
			foreach (wp_unslash($input['fxw_fee_tiers']) as $row) {
				if (!is_array($row)) {
					continue;
				}
	...
	public static function fee_for_distance($distance_km, $options = null)
	{
		...
		if (!isset($options['fxw_fee_mode']) || 'tiers' !== $options['fxw_fee_mode']) {
			return null;
		}
	...
	{
		$options = get_option('fxw_settings');
		$threshold = isset($options['fxw_free_delivery_threshold']) ? (float) $options['fxw_free_delivery_threshold'] : 0;
		if ($threshold <= 0) {
			return false;
		}
	...
	{
		$options = get_option('fxw_settings');
		$minimum = isset($options['fxw_minimum_order']) ? (float) $options['fxw_minimum_order'] : 0;
		if ($minimum <= 0) {
			return null;
		}
		...
		return sprintf(
			__('The minimum delivery order is %s — please add %s more to your cart.', 'foodxpress'),
	...
	public function render_minimum_order_notice()
	{
		...
		if (function_exists('wc_print_notice')) {
			wc_print_notice($error, 'error', array('fxw-minimum-order'));
		}
	}
}
