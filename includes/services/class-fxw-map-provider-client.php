 ...
 *
 * FXW_Mapping_Service owns caching, normalisation and the public API;
 ...
 */
class FXW_Map_Provider_Client
{

	/**
	 * Provider definition (output of FXW_Map_Providers::active()).
	 *
	 * @var array
	 */
	private $provider;
	...
	private function get($url)
	{
		if (!$this->is_allowed_url($url)) {
			return new WP_Error('invalid_endpoint', __('Map provider endpoint is not allowed.', 'foodxpress'));
		}

		$args = array(
			'timeout' => 15,
			'headers' => array(
				'User-Agent' => 'FoodXpress/' . FXW_VERSION . ' (WordPress; ' . home_url('/') . ')',
		...
		if (200 !== $code) {
			return new WP_Error('provider_http_error', sprintf(
				...
				__('Map provider returned HTTP %d.', 'foodxpress'),
				$code
			));
		}
		...
		if (!is_array($data)) {
			return new WP_Error('provider_bad_body', __('Map provider returned an unreadable response.', 'foodxpress'));
		}
	...
	private function reserve_slot()
	{
		...
		$lock = 'fxw_provider_slot_' . (isset($this->provider['id']) ? $this->provider['id'] : 'x');

		if (false !== get_transient($lock)) {
			return new WP_Error('provider_busy', __('Address lookups are busy for a moment — please try again, or drag the map pin instead.', 'foodxpress'));
		}
	...
	public function geocode($address)
	{
		...
		return new WP_Error('provider_unsupported', __('The configured map provider cannot geocode addresses.', 'foodxpress'));
	}
	...
	public function route($origin, $destination)
	{
		...
		if (!in_array('routing', (array) $this->provider['capabilities'], true)) {
			return new WP_Error('routing_unsupported', __('Provider has no road routing.', 'foodxpress'));
		}
	...
	{
		return new WP_Error('no_results', __('Could not find that location. Please try a different address or move the map pin.', 'foodxpress'));
	}
