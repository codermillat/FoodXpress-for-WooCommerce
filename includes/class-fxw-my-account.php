 ...
 *
 * Renders the FoodXpress-flavored dashboard widgets on the WooCommerce
 * My Account page:
...
class FXW_My_Account
{
	 ...
	 * FXW_Shortcodes and provides the layout shell; this stylesheet adds
	...
	public function enqueue_dashboard_assets()
	{
		...
		wp_enqueue_style(
			'fxw-my-account-dashboard',
			FXW_PLUGIN_URL . 'assets/css/my-account-dashboard.css',
			array('fxw-my-account'),
			FXW_VERSION
		);
	}
	...
	public function render_dashboard_widgets()
	{
		...
		?>
		<section class="fxw-dashboard" aria-label="<?php esc_attr_e('Account overview', 'foodxpress'); ?>">

			<header class="fxw-dashboard__welcome">
				<div class="fxw-dashboard__welcome-text">
					<span class="fxw-dashboard__greeting-eyebrow">
						<?php esc_html_e('Welcome back', 'foodxpress'); ?>
					</span>
					<h2 class="fxw-dashboard__greeting">
						<?php
						/* translators: %s: customer first name */
						printf(esc_html__('Hi, %s 👋', 'foodxpress'), esc_html($this->first_name($user)));
						?>
					</h2>
					<p class="fxw-dashboard__subgreeting">
						<?php
						if ($stats['active'] > 0) {
							/* translators: %d: number of in-flight orders */
							printf(esc_html(_n('You have %d order on the way.', 'You have %d orders on the way.', $stats['active'], 'foodxpress')), $stats['active']);
						} elseif ($stats['total'] > 0) {
							esc_html_e('Hungry again? Reorder your favourites in a tap.', 'foodxpress');
						} else {
							esc_html_e('Browse the menu and your first order is on the way to your door.', 'foodxpress');
						}
						?>
					</p>
				</div>
				<?php if ($last_completed) : ?>
					<a class="fxw-dashboard__reorder"
					   href="<?php echo esc_url(wp_nonce_url(add_query_arg('fxw_reorder', $last_completed->get_id()), 'fxw_reorder')); ?>">
						<span class="fxw-dashboard__reorder-icon" aria-hidden="true">↻</span>
						<?php esc_html_e('Reorder last order', 'foodxpress'); ?>
					</a>
				<?php else : ?>
					<a class="fxw-dashboard__reorder fxw-dashboard__reorder--ghost"
					   href="<?php echo esc_url(function_exists('wc_get_page_id') ? get_permalink(wc_get_page_id('shop')) : home_url('/shop')); ?>">
						<span class="fxw-dashboard__reorder-icon" aria-hidden="true">→</span>
						<?php esc_html_e('Browse menu', 'foodxpress'); ?>
					</a>
				<?php endif; ?>
			</header>

			<ul class="fxw-dashboard__stats" role="list">
				<li class="fxw-stat fxw-stat--active">
					<span class="fxw-stat__value"><?php echo esc_html((string) $stats['active']); ?></span>
					<span class="fxw-stat__label"><?php esc_html_e('On the way', 'foodxpress'); ?></span>
				</li>
				<li class="fxw-stat fxw-stat--completed">
					<span class="fxw-stat__value"><?php echo esc_html((string) $stats['completed']); ?></span>
					<span class="fxw-stat__label"><?php esc_html_e('Delivered', 'foodxpress'); ?></span>
				</li>
				<li class="fxw-stat fxw-stat--total">
					<span class="fxw-stat__value"><?php echo esc_html((string) $stats['total']); ?></span>
					<span class="fxw-stat__label"><?php esc_html_e('All orders', 'foodxpress'); ?></span>
				</li>
				<li class="fxw-stat fxw-stat--addresses">
					<span class="fxw-stat__value"><?php echo esc_html((string) $stats['addresses']); ?></span>
					<span class="fxw-stat__label"><?php esc_html_e('Addresses', 'foodxpress'); ?></span>
				</li>
			</ul>

			<div class="fxw-dashboard__grid">

				<section class="fxw-dashboard__panel fxw-dashboard__panel--orders" aria-labelledby="fxw-dashboard-recent-heading">
					<header class="fxw-dashboard__panel-header">
						<h3 class="fxw-dashboard__panel-title" id="fxw-dashboard-recent-heading">
							<?php esc_html_e('Recent orders', 'foodxpress'); ?>
						</h3>
						<a class="fxw-dashboard__panel-link"
						   href="<?php echo esc_url(function_exists('wc_get_endpoint_url') ? wc_get_endpoint_url('orders') : home_url('/my-account/orders/')); ?>">
							<?php esc_html_e('View all', 'foodxpress'); ?>
							<span aria-hidden="true">→</span>
						</a>
					</header>

					<?php if (empty($recent_orders)) : ?>
						<div class="fxw-dashboard__empty">
							<span class="fxw-dashboard__empty-icon" aria-hidden="true">🛒</span>
							<p class="fxw-dashboard__empty-title"><?php esc_html_e('No orders yet', 'foodxpress'); ?></p>
							<p class="fxw-dashboard__empty-text">
								<?php esc_html_e('Your first order will appear here. Tap "Browse menu" to get started.', 'foodxpress'); ?>
							</p>
						</div>
					<?php else : ?>
						<ul class="fxw-orders-list" role="list">
				...
				<aside class="fxw-dashboard__panel fxw-dashboard__panel--address" aria-labelledby="fxw-dashboard-address-heading">
					<header class="fxw-dashboard__panel-header">
						<h3 class="fxw-dashboard__panel-title" id="fxw-dashboard-address-heading">
							<?php esc_html_e('Default delivery address', 'foodxpress'); ?>
						</h3>
						<a class="fxw-dashboard__panel-link"
						   href="<?php echo esc_url(function_exists('wc_get_endpoint_url') ? wc_get_endpoint_url('edit-address') : home_url('/my-account/edit-address/')); ?>">
							<?php esc_html_e('Manage', 'foodxpress'); ?>
							<span aria-hidden="true">→</span>
						</a>
					</header>

					<?php if (empty($default_address)) : ?>
						<div class="fxw-dashboard__empty fxw-dashboard__empty--compact">
							<span class="fxw-dashboard__empty-icon" aria-hidden="true">📍</span>
							<p class="fxw-dashboard__empty-title"><?php esc_html_e('No address saved', 'foodxpress'); ?></p>
							<p class="fxw-dashboard__empty-text">
								<?php esc_html_e('Add a delivery address to speed up your next order.', 'foodxpress'); ?>
							</p>
							<a class="fxw-dashboard__empty-cta"
							   href="<?php echo esc_url(function_exists('wc_get_endpoint_url') ? wc_get_endpoint_url('edit-address') : home_url('/my-account/edit-address/')); ?>">
								<?php esc_html_e('Add address', 'foodxpress'); ?>
							</a>
						</div>
					<?php else : ?>
						<address class="fxw-address-card">
							<span class="fxw-address-card__label"><?php echo esc_html($default_address['label']); ?></span>
							<span class="fxw-address-card__name"><?php echo esc_html($default_address['name']); ?></span>
							<span class="fxw-address-card__line"><?php echo esc_html($default_address['line1']); ?></span>
							<?php if (!empty($default_address['line2'])) : ?>
								<span class="fxw-address-card__line"><?php echo esc_html($default_address['line2']); ?></span>
							<?php endif; ?>
							<?php if (!empty($default_address['city'])) : ?>
								<span class="fxw-address-card__line"><?php echo esc_html($default_address['city']); ?></span>
							<?php endif; ?>
						</address>
					<?php endif; ?>
				</aside>
	...
	private function render_recent_order_item($order)
	{
		...
		?>
		<li class="fxw-order-item">
			<div class="fxw-order-item__primary">
				<a class="fxw-order-item__number" href="<?php echo esc_url($order_url); ?>">
					<?php
					/* translators: %d: order number */
					printf(esc_html__('#%d', 'foodxpress'), (int) $order->get_order_number());
					?>
				</a>
				<span class="fxw-order-item__date"><?php echo esc_html($date_str); ?></span>
			</div>
			<div class="fxw-order-item__meta">
				<span class="fxw-status-pill <?php echo esc_attr($pill_class); ?>">
					<?php echo esc_html($label); ?>
				</span>
				<span class="fxw-order-item__total">
					<?php echo wp_kses_post($total); ?>
					<?php if ($item_count) : ?>
						<span class="fxw-order-item__count">
							<?php
							/* translators: %d: item count */
							printf(esc_html(_n('· %d item', '· %d items', $item_count, 'foodxpress')), $item_count);
							?>
						</span>
					<?php endif; ?>
				</span>
			</div>
			<div class="fxw-order-item__action">
				<a class="fxw-order-item__view" href="<?php echo esc_url($order_url); ?>" aria-label="<?php
					/* translators: %d: order number */
					printf(esc_attr__('View order %d', 'foodxpress'), (int) $order->get_order_number());
					?>">
					<?php esc_html_e('View', 'foodxpress'); ?>
				</a>
			</div>
		</li>
		<?php
	 ...
	 *
	 * @param string $status Order status slug (e.g. "processing", "fxw-in-kitchen").
	...
	{
		$map = array(
			'pending'         => 'fxw-status-pill--neutral',
			'on-hold'         => 'fxw-status-pill--neutral',
			'processing'      => 'fxw-status-pill--info',
			'fxw-in-kitchen'  => 'fxw-status-pill--accent',
			'fxw-assigned'    => 'fxw-status-pill--info',
			'fxw-picked-up'   => 'fxw-status-pill--info',
			'completed'       => 'fxw-status-pill--success',
			'cancelled'       => 'fxw-status-pill--danger',
			'refunded'        => 'fxw-status-pill--neutral',
			'failed'          => 'fxw-status-pill--danger',
		);
		return isset($map[$status]) ? $map[$status] : 'fxw-status-pill--neutral';
	}
	...
	private function collect_customer_stats($customer_id)
	{
		...
		$active_statuses = array('pending', 'on-hold', 'processing', 'fxw-in-kitchen', 'fxw-assigned', 'fxw-picked-up');
	...
	private function collect_default_address($customer_id)
	{
		...
		if (!empty($shipping['line1'])) {
			$chosen = $shipping;
			$label  = __('Shipping', 'foodxpress');
		} elseif (!empty($billing['line1'])) {
			$chosen = $billing;
			$label  = __('Billing', 'foodxpress');
		} else {
			return array();
		}
