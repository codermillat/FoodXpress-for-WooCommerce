...
class FXW_Map_Providers
{
	...
	public static function all()
	{
		return array(
			'google' => array(
				'label' => __('Google Maps', 'foodxpress'),
				'engine' => 'google',
				'requires_key' => true,
				'free_tier' => __('Requires a Google Cloud billing account. Free monthly credit covers small stores.', 'foodxpress'),
			...
			'osm' => array(
				'label' => __('OpenStreetMap (no key required)', 'foodxpress'),
				'engine' => 'leaflet',
				'requires_key' => false,
				'free_tier' => __('Completely free — no signup, no card. Runs on volunteer infrastructure: address lookups are throttled to one per second site-wide and the public routing server asks that busy stores not use it. Best for low-volume stores; switch to Geoapify if you outgrow it.', 'foodxpress'),
				'signup_url' => '',
				'capabilities' => array('map', 'geocode', 'routing'),
				...
				// lock in FXW_Map_Provider_Client, since a per-user rate
				// limit does nothing under concurrency.
				'geocode_min_interval' => 1,
			),
			'maptiler' => array(
				'label' => __('MapTiler', 'foodxpress'),
				'engine' => 'leaflet',
				'requires_key' => true,
				'free_tier' => __('Free key, no card required: 100k tile loads and 100k geocoding requests per month. No road routing — distance uses the straight-line estimate.', 'foodxpress'),
			...
			'geoapify' => array(
				'label' => __('Geoapify', 'foodxpress'),
				'engine' => 'leaflet',
				'requires_key' => true,
				'free_tier' => __('Free key, no card required: 3,000 requests per day covering tiles, geocoding and road routing in one provider.', 'foodxpress'),
	...
	public static function active_id($options = null)
	{
		...
		$id = isset($options['fxw_map_provider']) ? sanitize_key($options['fxw_map_provider']) : '';
		if (isset($all[$id])) {
			return $id;
		}

		// Never configured: infer from what the store already has.
		$google_key = isset($options['fxw_google_maps_api_key']) ? trim((string) $options['fxw_google_maps_api_key']) : '';
		if ('' !== $google_key) {
			return 'google';
		}
	 ...
	 * settings; every other provider uses `fxw_map_provider_key`.
	 *
	...
	public static function key_for($id, $options = null, $server = false)
	{
		...
		if ('google' === $id) {
			...
			return isset($options['fxw_google_maps_api_key']) ? trim((string) $options['fxw_google_maps_api_key']) : '';
		}

		return isset($options['fxw_map_provider_key']) ? trim((string) $options['fxw_map_provider_key']) : '';
	}
	...
	public static function road_factor($options = null)
	{
		...
		$factor = isset($options['fxw_road_distance_factor'])
			? (float) $options['fxw_road_distance_factor']
